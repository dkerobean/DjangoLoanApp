...
def indexPage(request):
    ...
    formatted_deposit = "{:,.2f}".format(total_deposit)

    # get sum of loans of users
    total_loans = LoanApplication.objects.filter(status='approved').aggregate(
                  Sum('amount'))['amount__sum']
    formatted_loans = "{:,.2f}".format(total_loans) \
        if total_loans is not None else "0.00"

    total_users = User.objects.count()

    number_of_loans = LoanApplication.objects.aggregate(
...
def loanApplicants(request):
    ...
    return render(
        request, 'admin_dashboard/loan_applicants/all_loans.html', context
        )
...
def viewApplication(request, pk):
    ...
    return render(
        request, 'admin_dashboard/loan_applicants/view_loan.html', context
        )
